...
class AksesRoleController extends Controller
{
    ...
    //         // Ambil nama permissions yang dimiliki oleh role
    //         $rolePermissions = $role->permissions->pluck('name')->toArray();

    //         return response()->json([
    //             'role' => $role,
    //             'allPermissions' => $permissions, // Semua permissions
    //             'rolePermissions' => $rolePermissions, // Permissions yang dimiliki role
    //         ]);
    ...
    //         \Log::info('Permissions received for update:', $request->input('permissions'));
    
    //         // Mengambil semua nama permissions yang dikirim dari request
    //         $permissionsToSync = $request->input('permissions');
    
    //         // Log sebelum sinkronisasi
    //         \Log::info('Syncing permissions to role: ' . $role->name, $permissionsToSync);
    
    //         // Sinkronkan permissions langsung dengan nama
    ...
    //     } catch (\Exception $e) {
    //         \Log::error('Error updating permissions:', ['message' => $e->getMessage(), 'trace' => $e->getTrace()]);
    //         return response()->json([
    //             'message' => 'Error updating permissions',
    //             'error' => $e->getMessage(),
    //         ], 500);
